...
npm run start
```
- Runs `scripts/startup-sync.sh` first, ensuring the working tree is reset to `origin/main` and dependencies are reinstalled before Metro launches.
...
npm run start:web
```
- Opens the Expo tunnel directly in web mode (after the auto-sync step).

```bash
npm run start:mobile
```
- Equivalent to `npm run start`, provided for clarity when pairing with mobile devices.
...
- If the full build fails the script automatically starts `npx expo start --web --tunnel` so the preview stays available while you investigate logs.

### One-command startup sync
To force a manual refresh of the repository state without starting Expo, run:

```bash
npm run sync:start
```

- Executes `scripts/startup-sync.sh`, which fetches `origin/main`, resets the working tree, and reinstalls dependencies in silent mode.
- Useful when preparing a CI agent or restoring the project immediately after cloning.
...
npm run build
```
- Executes the backend and web builds sequentially via `npm run build:backend && npm run build:web`.
- Produces `backend/dist/` and `dist/` artifacts without deploying to Netlify.
- Pair with `npm run deploy` to publish the freshly built output.
...
2. Deploy with `npm run deploy` (or run `npx netlify deploy --prod --dir dist`).
